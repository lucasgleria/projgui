...
def corrigir_erros_ocr(texto):
    # Substituir erros comuns
    correcoes = {
        "l": "1",  # Substituir 'l' por '1'
        "o": "0",  # Substituir 'o' por '0'
...
def encontrar_quantidade_e_medida(texto):
    ...
    padrao = r"(\d+(\.\d+)?\s*([gG]r?|mg|kg|ml|L)?)"
    ...
    if resultado:
        ...
        partes = re.split(r"\s*([gG]r?|mg|kg|ml|L)\b", quantidade_medida, flags=re.IGNORECASE)
