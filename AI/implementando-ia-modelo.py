...
def criar_tabela():
    ...
    cursor = conn.cursor()
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS implementando_ia (
...
def inserir_produto(nome_produto, quantidade, medida):
    conn = sqlite3.connect('implementando_ia.db')
    cursor = conn.cursor()
    cursor.execute('INSERT INTO implementando_ia (path, nome_produto, quantidade, medida) VALUES (?, ?, ?, ?)', (caminho_imagem, nome_produto, quantidade, medida))
    conn.commit()
    conn.close()
...
def obter_imagem():
    conn = sqlite3.connect('implementando_ia.db')
    cursor = conn.cursor()
    cursor.execute('SELECT path FROM implementando_ia ORDER BY id DESC LIMIT 1')
...
while True:
    ...
    if event == sg.WIN_CLOSED:
        ...
        if filepath:
            caminho_imagem = filepath  # Armazenar temporariamente o caminho da imagem
            exibir_imagem()
        ...
        if values['-NOME-'] and values['-QUANTIDADE-'] and values['-MEDIDA-']:
            inserir_produto(values['-NOME-'], values['-QUANTIDADE-'], values['-MEDIDA-'])
