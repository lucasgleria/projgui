...
baud_gates = ['300', '1200', '2400', '4800', '9600', '19200', '38400', '57600', '74880', '115200', '230400', '250000', '500000']
    ...
    def run_extract_nome(cursor):        
        ...
        if path:
            ...
            if nome is not None and isinstance(nome, list) and len(nome) > 0:
                ...
                add_product_window["nome"].update(nome_medicamento if nome_medicamento else "")
...
def extract_unidade_and_medida(add_product_window, conn, choose_window, attempt=1, timeout=5):
    global ultima_imagem_id
    stop_flag = [False]
    find_quantidade_and_medida_called = False  # Variável local para rastrear a execução
    ...
    def run_extract_unidade_and_medida(cursor, find_quantidade_and_medida_called):
        ...
        if path:
            ...
            if not find_quantidade_and_medida_called:
                quantidade, medida = find_quantidade_and_medida(text, timeout=5)
            ...
            if quantidade == "" or medida == "":
                if attempt == 1:
                    ...
                    sg.popup("Não consegui extrair as informações desejadas. \n\n Por favor, insira as informações manualmente." )
                    return quantidade, medida
            else:
                add_product_window["quantidade"].update(quantidade if quantidade else "")
    ...
    while not stop_flag[0]:
        run_extract_unidade_and_medida(conn.cursor(), find_quantidade_and_medida_called)
        if stop_flag[0]:
...
def find_quantidade_and_medida(text, timeout=5):
    ...
    def run_find_quantidade_and_medida():
        ...
        if resultado:
            ...
            partes = re.split(r"\s*(" + "|".join(medida_keywords) + r")\b", quantidade_medida, flags=re.IGNORECASE)
            ...
            if ser:
                ...
                while True:
                            ...
                            if add_event == sg.WINDOW_CLOSED:
                                    ...
                                    if filepath:
                                        ...
                                        if os.path.exists(filepath):
                                            ...
                                            if file_extension.lower() in valid_image_extensions:
                                                ...
                                                adicionar_produto(nome, registro, eixo_x, nota_fiscal_pdf_path, eixo_z, quantidade, medida, validade, filepath)
                                                ...
                                                sg.popup('Formato de imagem inválido!', title='Erro de Imagem', non_blocking=True, font=('Helvetica', 10), keep_on_top=True, auto_close_duration=3)
                                        else:
                                            sg.popup('Arquivo de imagem não encontrado!', title='Erro de Imagem', non_blocking=True, font=('Helvetica', 10), keep_on_top=True, auto_close_duration=3)
                                    else:
                                        # If no image is selected, add the product without an image
                                        adicionar_produto(nome, registro, eixo_x, nota_fiscal_pdf_path, eixo_z, quantidade, medida, validade, None)
                                        sg.popup('Produto adicionado sem imagem.', title='Produto Adicionado', non_blocking=True, font=('Helvetica', 10), keep_on_top=True, auto_close_duration=3)
                                        limpar_inputs(add_product_window, filepath)
                                ...
                                if selected_nota_fiscal_pdf_path:
                                    ...
                                    try:
                                        ...
                                        pil_image = pil_image.resize((50, 50), Image.ANTIALIAS if hasattr(Image, 'ANTIALIAS') else Image.LANCZOS)

                                        # Converte a imagem redimensionada de volta para o formato bytes
                                        image_bytes_resized = BytesIO()
                                        pil_image.save(image_bytes_resized, format='PNG')
                                        tk_pdf_image = ImageTk.PhotoImage(data=image_bytes_resized.getvalue())
                                        add_product_window['-PDF_PREVIEW-'].update(data=tk_pdf_image)
                                        add_product_window['text_pdf_info'].update(visible=True)
                                ...
                                if selected_nota_fiscal_pdf_path:
                                    try:
                                        view_selected_pdf_window(selected_nota_fiscal_pdf_path)
                    ...
                    #         print("Erro em Calibrar: " + str(e))
                        ...
                        try:
                            calibrar = f"G28 Y Z X" + '\n' # G28 falar com GUi para corrigir
                            ser.write(calibrar.encode())
                        except Exception as e:
                            sg.popup_error('Nenhuma porta serial conectada.\n\n Impossível fazer a comunicação.', title='Produtos Não Encontrados',
                                           non_blocking=True, font=('Helvetica', 10), keep_on_top=True)
                ...
                sg.popup_error("Erro ao configurar a porta serial. Certifique-se de que a porta e a taxa de banda estão corretas.")
                print('Erro critico')
                sys.exit()
