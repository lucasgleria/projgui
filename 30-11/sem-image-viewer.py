...
def adicionar_produto(nome, registro, eixo_x, nota_fiscal, eixo_z, quantidade, medida_da_quantidade, imagem_path):
    global selected_image_path

    data_criada = datetime.datetime.now()

    # Use selected_image_path instead of imagem_path
    cursor.execute('INSERT INTO produtos (nome, registro, eixo_x, nota_fiscal, eixo_z, quantidade, medida_da_quantidade, imagem_path, data_criada) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
                   (nome, registro, eixo_x, nota_fiscal, eixo_z, quantidade, medida_da_quantidade, selected_image_path, data_criada))
    conn.commit()
...
def get_product_info(product_name):
    ...
    if product_info:
        ...
        # You can return the image path as well
...
def info_product_layout():
    return [
        ...
        [sg.Image(key='-PRODUCT_IMAGE-', size=(300, 300))],
            ...
            if add_event == sg.WINDOW_CLOSED:
                ...
                if imagem_path and os.path.exists(imagem_path) and os.path.isfile(imagem_path):
                    ...
                    if file_extension.lower() in valid_image_extensions:
                        ...
                        with open(image_path_full, 'rb') as image_file:
                    ...
                    if selected_row:
                        ...
                        if product_info:
                            ...
                            if image_path and os.path.exists(image_path):
                                ...
                                pil_image = pil_image.resize((300, 300), Image.ANTIALIAS if hasattr(Image, 'ANTIALIAS') else Image.LANCZOS)
                                tk_image = ImageTk.PhotoImage(pil_image)
                                info_product_window['-PRODUCT_IMAGE-'].update(data=tk_image)
